...
class LocalComposer extends StatefulWidget {
  ...
  @override
  State<LocalComposer> createState() => _LocalComposerState();
}

class _LocalComposerState extends State<LocalComposer> {
  final _key = GlobalKey();
  // late final TextEditingController _textController;
  late final FocusNode _focusNode;
  late final ValueNotifier<bool> _hasTextNotifier;
  late final FlutterTaggerController _textController;

  @override
  void initState() {
    super.initState();
    _textController = FlutterTaggerController();
  ...
  @override
  void didUpdateWidget(covariant LocalComposer oldWidget) {
              ...
              children: [
                ...
                Expanded(
                  child: FlutterTagger(
                    ...
                    builder: (context, containerKey) => TextField(
                              ...
                              itemBuilder: (_, index) {
                                ...
                                return ListTile(
                                  leading: CircleAvatar(
                                    child: Text(
                                      getAvatarNameFromTitle(user['user']!),
                                    ),
                                  ),
...
String getAvatarNameFromTitle(String data) {
  final splitted = data.split('-').first;
  final secondSplit = splitted
      .split(' ')
      .where((element) => element != '')
      .toList();
  secondSplit.removeAt(0);
  return secondSplit.map((e) => e[0]).take(3).join();
}
